...
const formation = ({data}) => {
  ...
  <GatsbyImage
	...
	<h1 className="text-center text-orange-z text-3xl font-black">{data.datoCmsListesFormation.nomDeLaFormation}</h1>
  ...
  <section className="flex flex-col gap-10 md:grid grid-cols-2 md:gap-20">
    <div className="flex flex-col gap-5">
      <h2 className="text-2xl text-bleu-z font-black">{data.datoCmsListesFormation.titreUn}</h2>
      <div dangerouslySetInnerHTML={{ __html: data.datoCmsListesFormation.publicCible }}/> 

    </div>
    <div className="flex flex-col gap-5">
    <h2 className="text-2xl text-bleu-z font-black">{data.datoCmsListesFormation.titreDeux}</h2>
    <div dangerouslySetInnerHTML={{ __html: data.datoCmsListesFormation.conditionDacces }}/> 

    </div>
  </section>
  <section className="flex flex-col gap-5">
  <h2 className="text-2xl text-bleu-z font-black">{data.datoCmsListesFormation.titreTrois}</h2>
  <div dangerouslySetInnerHTML={{ __html: data.datoCmsListesFormation.objectifCommunTousLesCoursDeFle }}/> 
</section>
<section className="flex flex-col gap-5">
<h2 className="text-2xl text-bleu-z  font-black">{data.datoCmsListesFormation.titreQuatre}</h2>
<div dangerouslySetInnerHTML={{ __html: data.datoCmsListesFormation.organisation }}/> 
<h3 className="text-xl text-bleu-z font-black">{data.datoCmsListesFormation.titreCinq}</h3>
